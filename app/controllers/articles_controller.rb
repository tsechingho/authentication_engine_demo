class ArticlesController < ApplicationController
  ...
  before_filter :find_current_department
  ...
  def new_article
    @article = @current_department.articles.build(params[:article])
  ...
  def find_article
    @article = @current_department.articles.find(params[:id])
  rescue ActiveRecord::RecordNotFound
    redirect_to articles_path
  end

  def find_created_article
    @article = @current_department.articles.with_state(:created).find(params[:id])
  rescue ActiveRecord::RecordNotFound
    redirect_to articles_path
  end

  def find_confirmed_article
    @article = @current_department.articles.with_state(:confirmed).find(params[:id])
  rescue ActiveRecord::RecordNotFound
    redirect_to articles_path
  end
end
